    ...
    bool reentrant_thread) -> void 
{
  ...
  while (!reentrant_thread || graph_task->outstanding_tasks_ > 0) 
  {
    ...
    if (base_owner != NO_DEVICE && base_owner != worker_device && gt_completed) 
    {
    ...
    std::shared_ptr<Node> graph_root) 
{
  ...
  else 
  {
    ...
    if (current_depth >= max_recursion_depth_) 
    {
      ...
      // If reached the max depth, switch to a differet thread
...
void GraphTask::init_to_execute(Node& graph_root, const edge_list& outputs) 
{
  ...
  //     is_needed[fn] = any(compute_is_needed(next_edge) for next_edge in fn.next_edges)
  //   return is_needed[fn]
  struct Frame 
  {
