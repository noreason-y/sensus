    ...
    THCTensor* target) 
{
  ...
  else if (input->dim() == 2) 
  {
    ...
    TORCH_CHECK(
        !target->is_empty() && (target->dim() == 2) && (target->size(0) == nframe) && (target->size(1) == dim),
  ...
  else if (input->dim() == 2)
  {
    ...
    if (reduction != at::Reduction::None)
    {
      ...
      THCTensor_(set1d)(
          ...
          output, 
          0, 
          ScalarConvert<accreal, scalar_t>::to(THCTensor_(sumall)(state, output_tmp)));
    ...
    int64_t reduction)
{
  ...
  else if (gradInput->dim() == 2)
  {
    ...
    THArgCheck(
        ...
        "inconsistent target size");
    THArgCheck(
        !istarget->is_empty() && (istarget->dim() == 2) && (istarget->size(0) == nframe) 
          && (istarget->size(1) == dim), 
          3, 
